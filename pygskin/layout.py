...
class Fill(Layout):
    ...
    def __call__(self, bounds: Rect, items: LayoutItems, **kwargs) -> None:
        if items:
            bounds = Rect(bounds)
            padded_topleft = Vector2(0)
            if padding := kwargs.get("padding"):
                padding = Padding(padding)
                padded_topleft.update(padding.left, padding.top)
            ...
            if self.direction == Direction.VERTICAL:
                ...
                slot = pygame.Rect(padded_topleft, (bounds.width, offset.y))
            else:
                offset = Vector2(bounds.width // len(items), 0)
                slot = pygame.Rect(padded_topleft, (offset.x, bounds.height))

            for item in items:
                if item.rect is None:
                    item.rect = slot.copy()
                item.rect.update(slot)

                if isinstance(item, Container):
                    item.layout(
                        item.rect,
                        item.children,
                        padding=getattr(item, "style", {}).get("padding"),
                    )
